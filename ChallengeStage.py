...
class ChallengeScreen:
    """
    Class representing the Challenge Screen, which displays a challenging stage
    with animations and game-related information.

    Attributes:
        window (pygame.Surface): The surface to draw the Challenge Screen on.
        assets (Assets): The game assets, including images for cars and objects.
        challenge_num (int): The current challenge stage number.
    ...
    def __init__(self, surface):
        """
        Initialize the ChallengeScreen.

        :param surface: The surface to draw the Challenge Screen on.
        """
        py.init()
        self.window = surface
        self.assets = Assets()  # Load game assets
        self.challenge_num = 1  # Current challenge number
        ...
        self.y_offset = 100  # Initial vertical offset for text
        self.anim_index = 0  # Index to track the current animation step

    def input(self):
        """
        Handle user input to control the Challenge Screen.
        """
    ...
    def draw(self):
        """
        Draw the Challenge Screen, including text and objects.
        """
        ...
        if self.isAnimate:
            ...
            for i in range(len(self.text)):
                ...
                if i == 2 or i == 3:
                    t_box_rect = t_box.get_rect(midleft=(gs.SCREEN_WIDTH / 2, self.y_offset))
                else:
            ...
            self.y_offset = 100  # Reset vertical offset
    ...
    def animate(self, s, y_cord, color):
        ...
        for i in range(len(s) + 1):
            ...
            for j in range(self.anim_index):
                ...
                if j >= 2 and j <= 3:
                    prev_text_rect = prev_text.get_rect(midleft=(gs.SCREEN_WIDTH / 2, y_offset))
                else:
            ...
            if self.anim_index == 2 or self.anim_index == 3:
                self.text_rect = self.rendered_text.get_rect(midleft=(gs.SCREEN_WIDTH / 2, y_cord))
            else:
    ...
    def runscreen(self):
        """
        Run the Challenge Screen loop, handling input and drawing the screen.
        """
        while self.run:
